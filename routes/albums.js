...
const albumMiddleware = (viewing = false) => async (req, res, next) => {
  ...
  if (
    permissionRequired &&
    (!req.user || (album.userId !== req.user.id && !req.user.superuser))
  )
...
router.post(
  '/:id',
  isAuthenticated,
  albumMiddleware(false),
  async (req, res, next) => {
    if (req.body.hasOwnProperty('private'))
      req.album.private = +req.body.private;
    if (req.body.hasOwnProperty('title')) req.album.title = req.body.title;
