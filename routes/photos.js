...
const photoMiddleware = (viewing = false) => async (req, res, next) => {
    ...
    if (permissionRequired && (!req.user || (photo.userId !== req.user.id && !req.user.superuser)))
...
router.get('/:id/file/:size', photoMiddleware(true), (req, res, next) => {
    ...
    res.sendFile(path.resolve(destination, isThumbnail ? req.photo.thumbnailFilename : req.photo.filename), {
        headers: {
            'Content-Type': isThumbnail ? req.photo.thumbnailMimetype : req.photo.mimetype,
        }
    });
});

router.post('/:id/file/:size', isAuthenticated, photoMiddleware(false), upload.single('file'), async (req, res, next) => {
    if (!req.file)
        return next(new Error('File missing.'));

    const isThumbnail = req.params['size'] === 'thumbnail';

    await unlink(path.resolve(destination, isThumbnail ? req.photo.thumbnailFilename : req.photo.filename));
